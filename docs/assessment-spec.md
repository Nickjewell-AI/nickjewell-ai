...
TIER 3: THE TASTE TEST (3 scenarios from pool of 9, maturity-routed, ~1 min)
  ...
  ↓
RESULTS → Contact form (name/email/company/role)
  → Executive brief streams via Claude Opus 4.6 (conditioned by industry + company size)
```
...
### Executive Brief
Generated by Claude Opus 4.6 via streaming. The brief system prompt is conditioned by:
- **Industry (P3):** 5 distinct conditioning blocks with industry-specific action language
- **Company Size (P7):** 5 distinct conditioning blocks calibrating action specificity (from "export to Google Sheets this week" for a 20-person startup to "get your CTO and risk officer in the same room" for a 5,000+ enterprise)
...
Brief is gated behind contact form (name, email, company, role). Rate limited: 3 briefs/IP/day, 100 global/day.

**Deterministic fallback:** If Opus fails, brief generation shows error with retry option. Assessment results are always available regardless of API state.
...
All API calls route through `/functions/api-proxy.js`. API key stored as Cloudflare Worker environment variable. Never exposed to client. Rate limiting via D1.
...
### assessment_results
id, created_at, all_responses (JSON), scores (JSON), taste_signature, verdict, binding_constraint, contact_info (JSON), brief_generated, is_test, taste_normalized (JSON), scenario_set (TEXT)

### assessment_feedback
id, assessment_id, created_at, sentiment, feedback_text, source_name, source_company, source_channel, source_role, signal_type

### brief_ip_counter
ip, date, count

### email_log
id, created_at, to_email, template_type, status, error
