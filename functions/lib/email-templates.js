...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <tr><td style="padding:32px 0 24px;">
    <p style="color:#c8965a;font-family:Georgia,'Times New Roman',serif;font-size:18px;font-weight:600;margin:0;">Jewell Assessment</p>
  </td></tr>
  ...
  <tr><td style="padding-bottom:28px;">
    <a href="https://www.nickjewell.ai/framework/#${bindingConstraint || ''}" style="color:#c8965a;font-size:14px;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;text-decoration:none;">Deep dive: Why ${constraintName} breaks AI initiatives &rarr;</a>
  </td></tr>
...
/**
 * Build the immediate results-only email (no brief content).
 * Sent on capture gate submission so users who close the tab still get results.
 */
export function buildResultsOnlyEmail(data) {
  const {
    firstName,
    verdict,
    composite,
  ...
  const preheader = `${(verdict || '').toUpperCase()} — ${composite}/100. Your binding constraint is ${constraintName}.`;
  const preheaderPad = '&nbsp;'.repeat(80);
  ...
  const actionItems = actions
    .map((a, i) => `<li style="color:#1a1a1a;font-size:14px;line-height:1.6;margin-bottom:8px;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;">${a}</li>`)
    .join('');
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="padding:20px;">
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
          <tr>
            <td style="width:12px;height:12px;border-radius:50%;background:${verdictColor};"></td>
            <td style="padding-left:8px;color:${verdictColor};font-size:16px;font-weight:700;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;text-transform:uppercase;letter-spacing:0.04em;">${(verdict || '').toUpperCase()}</td>
            <td style="padding-left:12px;color:#1a1a1a;font-size:16px;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;">${composite != null ? composite + '/100' : ''}</td>
          </tr>
        </table>
  ...
  <!-- Binding Constraint -->
  <tr><td style="padding-bottom:24px;">
    <p style="color:#c8965a;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;margin:0 0 8px;">Binding Constraint: ${constraintName}</p>
    <p style="color:#1a1a1a;font-size:15px;line-height:1.6;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;margin:0;">${constraintExplanation || ''}</p>
  </td></tr>

  <!-- Action Plan -->
  ${actionItems ? `
  <tr><td style="padding-bottom:28px;">
    <p style="color:#c8965a;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;margin:0 0 12px;">Your Action Plan</p>
    <ol style="padding-left:20px;margin:0;">${actionItems}</ol>
  </td></tr>` : ''}

  <!-- Brief coming soon notice -->
  <tr><td style="padding-bottom:28px;">
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e0ddd8;border-radius:4px;background:#f8f7f5;">
      <tr><td style="padding:16px 20px;">
        <p style="color:#1a1a1a;font-size:14px;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;margin:0;line-height:1.5;">Your personalized executive brief is being generated and will arrive in a separate email shortly.</p>
      </td></tr>
    </table>
  </td></tr>
  ...
  <!-- Share CTA -->
  <tr><td style="padding-bottom:24px;">
    <p style="margin:0;"><a href="https://www.nickjewell.ai/assessment" style="color:#c8965a;font-size:14px;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;text-decoration:none;">Share the assessment &rarr; nickjewell.ai/assessment</a></p>
  </td></tr>
